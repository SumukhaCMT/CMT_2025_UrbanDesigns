...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['title']) && isset($_POST['section_heading'])) {
    ...
    if (!empty($title) && strlen($title) >= 2 && strlen($title) <= 50 && !empty($section_heading) && strlen($section_heading) >= 1 && strlen($section_heading) <= 50) {
        try {
            $update_stmt = $pdo->prepare("UPDATE faq_title SET title = :title, section_heading=:section_heading WHERE id = :id");
            $update_stmt->execute([
                ':title' => $title,
            ...
            echo "<script>alert('FAQ Heading, Title Updated Successfully!'); 
...
$features_title_stmt = $pdo->prepare("SELECT * FROM faq_title WHERE id = 1");
...
if (isset($_POST['delete_id'])) {
    ...
    echo "<script>alert('FAQ deleted successfully!'); 
    window.location.href = 'faq.php';</script>";
    exit();
}
?>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            <div class="button-container">
                                                                <a href="edit-faq.php?id=<?php echo $faq['id']; ?>"
